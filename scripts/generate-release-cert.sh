...
# for SnipSnap release builds.
#
# This certificate is used in CI to sign release builds so that macOS TCC
# recognises updates as the same app and preserves granted permissions
# (Screen Recording, Accessibility, Input Monitoring).
#
# Usage:
#   ./scripts/generate-release-cert.sh [--name "Cert Name"] [--output dir]
#
# After running, add the printed base64 string and password as GitHub secrets:
#   SIGNING_CERTIFICATE_P12       — base64-encoded .p12 file
#   SIGNING_CERTIFICATE_PASSWORD  — the password printed by this script
